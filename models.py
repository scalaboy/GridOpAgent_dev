...
from sublayer import MultiHeadAttention, PositionwiseFeedForward,Edge_GNNNet,get_edge_from_distance


class GATLayer_base(nn.Module):
    ...
    def forward(self, x, adj):
        print('input0',x.shape,adj.shape)
        x = self.slf_attn(x, adj)
        print('input1',x.shape,adj.shape)
        x = self.pos_ffn(x)
        print('input2',x.shape,adj.shape)
        return x


class GATLayer(nn.Module):
    def __init__(self,edge_index,edge_attr, output_dim, nheads, dropout=0):
        super(GATLayer, self).__init__()
        #print('in GATLayer create',adj)
        
        self.slf_attn = MultiHeadAttention(nheads, output_dim, output_dim//4, dropout=dropout)
        self.edge_ffn = Edge_GNNNet(edge_index,edge_attr,output_dim)
...
class Actor(nn.Module):
    def __init__(self,edge_index,edge_attr, output_dim, nheads, node, action_dim, use_order, n_sub, dropout=0, init_w=3e-3, log_std_min=-10, log_std_max=1):
        super(Actor, self).__init__()
        self.gat1 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.gat2 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.gat3 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.n_sub = n_sub
...
class SoftQ(nn.Module):
    def __init__(self,edge_index,edge_attr, output_dim, nheads, node, action_dim, use_order, n_sub, dropout=0, init_w=3e-3):
...
class DoubleSoftQ(nn.Module):
    def __init__(self, edge_index,edge_attr, output_dim, nheads, node, action_dim, use_order, n_sub, dropout=0, init_w=3e-3):
        ...
        self.Q1 = SoftQ(edge_index,edge_attr,output_dim, nheads, node, action_dim, use_order, n_sub, dropout, init_w)
        self.Q2 = SoftQ(edge_index,edge_attr,output_dim, nheads, node, action_dim, use_order, n_sub, dropout, init_w)
...
class EncoderLayer(nn.Module):
    def __init__(self, edge_index,edge_attr,input_dim, output_dim, nheads, node, dropout=0):
        super(EncoderLayer, self).__init__()
        self.linear = nn.Linear(input_dim, output_dim)
        self.gat1 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.gat2 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.gat3 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.gat4 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.gat5 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)
        self.gat6 = GATLayer(edge_index,edge_attr,output_dim, nheads, dropout)

    def forward(self, x, adj):
        x = self.linear(x)
        print('begin',x.shape,adj.shape)
        x = self.gat1(x, adj)
        print('begin gat1',x.shape,adj.shape)
        x = self.gat2(x, adj)
        print('begin gat2',x.shape,adj.shape)
        x = self.gat3(x, adj)
        x = self.gat4(x, adj)
        x = self.gat5(x, adj)
        x = self.gat6(x, adj)
