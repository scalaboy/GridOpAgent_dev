...
#from models_base import DoubleSoftQ, EncoderLayer, Actor
...
class Agent(BaseAgent):
    def __init__(self, env, **kwargs):
        ...
        #add 20220330
        obs_tmp = env.reset()
        self.adj = (torch.FloatTensor(obs_tmp.connectivity_matrix()) + torch.eye(int(obs_tmp.dim_topo))).to(self.device)
        print(f'N: {self.node_num}, O: {self.input_dim}, S: {self.state_dim}, A: {self.action_dim},999adj:({self.adj}), ({self.order_dim})')
        print(kwargs)
        edge_index,edge_attr = get_edge_from_distance(self.adj,self.device)

        self.emb = EncoderLayer(edge_index,edge_attr,self.input_dim, self.state_dim, self.nheads, self.node_num, self.dropout).to(self.device)
        self.temb = EncoderLayer(edge_index,edge_attr,self.input_dim, self.state_dim, self.nheads, self.node_num, self.dropout).to(self.device)
        self.Q = DoubleSoftQ(edge_index,edge_attr, self.state_dim, self.nheads, self.node_num, self.action_dim,
                        self.use_order, self.order_dim, self.dropout).to(self.device)
        self.tQ = DoubleSoftQ(edge_index,edge_attr,self.state_dim, self.nheads, self.node_num, self.action_dim,
                        self.use_order, self.order_dim, self.dropout).to(self.device)
        self.actor = Actor(edge_index,edge_attr,self.state_dim, self.nheads, self.node_num, self.action_dim,
                        self.use_order, self.order_dim, self.dropout).to(self.device)
