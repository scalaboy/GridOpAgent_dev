...
class GATLayer_iter(nn.Module):
    def __init__(self, output_dim, nheads, dropout=0):
        super(GATLayer, self).__init__()
        self.slf_attn = MultiHeadAttention(nheads, output_dim, output_dim//4, dropout=dropout)
        self.edge_ffn = Edge_GNNNet(output_dim, output_dim, dropout=dropout)
    
    def forward(self, x, adj):
        
        x = self.slf_attn(x, adj)
        edge_index,edge_attr = get_edge_from_distance(adj)
        x = self.edge_ffn(x,edge_index,edge_attr)
        return x
